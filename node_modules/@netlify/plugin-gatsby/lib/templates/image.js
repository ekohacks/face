...
function generateURLFromBase64EncodedPath(path) {
    ...
    const decodedUrl = buffer_1.Buffer.from(encodedUrl, 'base64').toString('utf8');
    const decodedArgs = buffer_1.Buffer.from(encodedArgs, 'base64').toString('utf8');
    let sourceURL;
    try {
...
const handler = async (event) => {
    const QUERY_PARAM_PATTERN = /^\/\.netlify\/functions\/__image\/image_query_compat\/?$/i;
    const { pathname } = new URL(event.rawUrl);
    const match = pathname.match(QUERY_PARAM_PATTERN);
    let newURL;
    if (match) {
        // Extract the query parameters
        const { url: uParam, cd: cdParam, args: argsParam, } = event.queryStringParameters;
