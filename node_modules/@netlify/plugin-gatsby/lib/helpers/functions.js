...
const setupImageCdn = async ({ constants, netlifyConfig, }) => {
    const { GATSBY_CLOUD_IMAGE_CDN, NETLIFY_IMAGE_CDN } = netlifyConfig.build.environment;
    if (NETLIFY_IMAGE_CDN !== `true` &&
        GATSBY_CLOUD_IMAGE_CDN !== '1' &&
        GATSBY_CLOUD_IMAGE_CDN !== 'true') {
        return;
    }
    await (0, fs_extra_1.ensureDir)(constants.INTERNAL_FUNCTIONS_SRC);
    ...
    if (NETLIFY_IMAGE_CDN === `true`) {
        await (0, fs_extra_1.copyFile)((0, pathe_1.join)(__dirname, '..', '..', 'src', 'templates', 'image.ts'), (0, pathe_1.join)(constants.INTERNAL_FUNCTIONS_SRC, '__image.ts'));
    ...
    else if (GATSBY_CLOUD_IMAGE_CDN === '1' ||
        GATSBY_CLOUD_IMAGE_CDN === 'true') {
        netlifyConfig.redirects.push({
            from: `/_gatsby/image/:unused/:unused2/:filename`,
            // eslint-disable-next-line id-length
